...
import com.gxuc.runfast.shop.util.AMapUtil;
import com.gxuc.runfast.shop.util.SharePreferenceUtil;
import com.gxuc.runfast.shop.util.ToastUtil;
import com.gxuc.runfast.shop.view.WalkRouteOverlayNew;
...
//        loginResponse = BaseApplication.context().getLoginResponse();
		if (!TextUtils.isEmpty(SharePreferenceUtil.getInstance().getStringValue(CustomConstant.POINTLAT)) && !TextUtils.isEmpty(SharePreferenceUtil.getInstance().getStringValue(CustomConstant.POINTLON))) {
			...
			Double userLat = Double.valueOf(SharePreferenceUtil.getInstance().getStringValue(CustomConstant.POINTLAT));
//        Double userLat = Double.valueOf("23.3528547955");
			Double userLng = Double.valueOf(SharePreferenceUtil.getInstance().getStringValue(CustomConstant.POINTLON));

//        Double userLng = Double.valueOf("110.0644254684");
			endLat = Double.valueOf(getIntent().getStringExtra("lat"));
			endLng = Double.valueOf(getIntent().getStringExtra("lng"));
			mStartPoint = new LatLonPoint(userLat, userLng);
	...
	private void setfromandtoMarker() {
		...
		.icon(BitmapDescriptorFactory.fromResource(R.drawable.icon_map_start)));
		aMap.addMarker(new MarkerOptions()
			...
			if (result != null && result.getPaths() != null) {
				if (result.getPaths().size() > 0) {
					...
					final WalkPath walkPath = mWalkRouteResult.getPaths()
							.get(0);
					WalkRouteOverlayNew walkRouteOverlay = new WalkRouteOverlayNew(
							this, aMap, walkPath,
