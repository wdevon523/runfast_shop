...
public class ListContainer extends LinearLayout {
    ...
    public TypeNewAdapter typeAdapter;
    ...
    public FoodNewAdapter foodAdapter;
    ...
    public ListContainer(Context context, @Nullable AttributeSet attrs) {
        ...
        typeAdapter = new TypeNewAdapter(mContext,((BusinessNewActivity) mContext).getTypes());
        View view = new View(mContext);
        view.setMinimumHeight(ViewUtils.dip2px(mContext, 80));
...
//		foodBeanList = BaseUtils.getDatas(mContext);
        foodBeanList = ((BusinessNewActivity) mContext).getFoodBeens();
        recyclerView2.addOnItemTouchListener(new OnItemClickListener() {
            @Override
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                super.onItemChildClick(adapter, view, position);
                if (view.getId() == R.id.rl_spec) {

//                    ((BusinessNewActivity) mContext).showSpecDialog((FoodBean) adapter.getData().get(position));
                    ((BusinessNewActivity) mContext).showSpecDialog(position);

                } else if (view.getId() == R.id.food_main) {
//					Intent intent = new Intent(mContext, DetailActivity.class);
//					intent.putExtra("food", (FoodBean) adapter.getData().get(position));
//					intent.putExtra("position", position);
//					mContext.startActivity(intent);
//					((Activity) mContext).overridePendingTransition(android.R.anim.fade_in, android.R.anim.fade_out);
                    ((BusinessNewActivity) mContext).showGoodDetail(position);
                }
            }
    ...
    public void setAddClick(AddWidget.OnAddClick onAddClick) {
        foodAdapter = new FoodNewAdapter(mContext, foodBeanList, onAddClick);
        View view = new View(mContext);
        view.setMinimumHeight(ViewUtils.dip2px(mContext, 80));
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (move) {
                    ...
                    if (transInfoView != null && transInfoView.getTag() != null) {
                        ...
                        if (transViewStatus == FoodNewAdapter.HAS_STICKY_VIEW) {
                            if (transInfoView.getTop() > 0) {
                                stickView.setTranslationY(dealtY);
                            } else {
                                stickView.setTranslationY(0);
                            }
                        } else if (transViewStatus == FoodNewAdapter.NONE_STICKY_VIEW) {
                            stickView.setTranslationY(0);
                        }
                    }
                }
